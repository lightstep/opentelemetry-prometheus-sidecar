...
func TestCache_Get(t *testing.T) {
	metrics := []prometheus.APIMetadata{
	...
	var handler func(qMetric, qMatch string) *prometheus.APIResponse
	...
	expect := func(want prometheus.APIMetadata, got *config.MetadataEntry) {
		if !reflect.DeepEqual(want, prometheus.APIMetadata{
			Metric: got.Metric,
			Type:   got.MetricType,
			Help:   got.Help,
		}) {
	...
	handler = func(qMetric, qMatch string) *prometheus.APIResponse {
		if qMetric != "" {
		...
		return &prometheus.APIResponse{Status: "success", Data: metrics[:4]}
	}
	...
	handler = func(qMetric, qMatch string) *prometheus.APIResponse {
		...
		return &prometheus.APIResponse{Status: "success", Data: metrics[4:5]}
	}
	...
	handler = func(qMetric, qMatch string) *prometheus.APIResponse {
		...
		return &prometheus.APIResponse{Status: "success", Data: metrics[5:6]}
	}
	md, err = c.Get(ctx, "prometheus", "localhost:9090", "metric6")
	if err != nil {
		t.Fatal(err)
	}
	expect(prometheus.APIMetadata{Metric: "metric6", Type: textparse.MetricTypeUnknown, Help: "help_metric6"}, md)
	...
	handler = func(qMetric, qMatch string) *prometheus.APIResponse {
		...
		return &prometheus.APIResponse{Status: "error", ErrorType: apiErrorNotFound, Error: "does not exist"}
	}
	...
	handler = func(qMetric, qMatch string) *prometheus.APIResponse {
		return &prometheus.APIResponse{Status: "success", Data: metrics}
	}
