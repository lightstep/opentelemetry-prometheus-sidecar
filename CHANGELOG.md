...
- New metric `sidecar.points.skipped` counts points that were not processed due to filters or cumulative resets
- New metric `sidecar.series.defined` counts the number of series refs defined in the WAL
- New metric `sidecar.metadata.lookups` counts the number of metadata lookups (with error=true/false)
- New metric `sidecar.cumulative.missing_resets` counts the number of points that were not processed due to cumulative resets
- New metric `sidecar.series.current` reports the current number of series (with status=live/filtered/invalid)

### Changed

- Series cache remembers points that were filtered in order to correctly count points that are dropped.
- Metric `sidecar.metadata.fetch.duration` has new `mode` label for single and batch requests.
